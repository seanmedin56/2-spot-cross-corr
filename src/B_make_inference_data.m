...
%%% Cleaning Parameters
big_jump1 = prctile([trace_struct.fluo],99.5);
...
for i = 1:length(trace_struct) 
    ...
    trace3_nans = isnan(trace3);
    if length(trace3_nans) == sum(trace3_nans)
        trace3 = zeros(1, length(trace3));
    end
    %Look for clusters of 6 or more NaNs
    kernel = [1,1,1,1,1];
    tn_conv = conv(kernel,trace3_nans);
    tn_conv = tn_conv(3:end-2);
    z_ids = find(tn_conv==5);
    z_ids = unique([z_ids-1 z_ids z_ids+1]); % get set of z_ids    
    trace3(z_ids) = 0;
