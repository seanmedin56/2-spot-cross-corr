function gillespie = synthetic_two_spot_gillespie(seq_length, alpha, ...
    ...
    % w: cell array with memories of different fluorescing genes
    ...
    % traces
    fluo = cell([1 length(w)]);
    fluo_MS2 = cell([1 length(w)]);
    fluo_noise = cell([1 length(w)]);
    fluo_MS2_noise = cell([1 length(w)]);
    fluo_MS2_orig_noise = cell([1 length(w)]);
    for wi = 1:length(w)
        ...
        fluo_cum_interp_shift = [zeros([1, w{wi}]), fluo_cum_interp(1:(end-w{wi}))];
        fluo{wi} = fluo_cum_interp - fluo_cum_interp_shift;
        ...
        for k = 1:seq_length
            ...
            for i = 1:(length(naive_window)-1)
                ...
                fluo_MS2_orig(k) = fluo_MS2_orig(k) + r_emission(naive_window(i)) * ...
                              ms2_loading_coeff_integral(alpha, w{wi}, deltaT, t1, t2);
        ...
        fluo_noise{wi} = fluo{wi} + gauss_noise;
